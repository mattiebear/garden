defmodule GardenServerWeb.UserAuthTest do
  use GardenServerWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias GardenServer.Accounts
  alias GardenServer.Accounts.Scope
  alias GardenServerWeb.UserAuth

  import GardenServer.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, GardenServerWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      GardenServerWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        GardenServerWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
